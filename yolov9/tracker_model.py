...
class TrackerModel:
    def __init__(self, source, video_stat):
    ...
    def draw_boxes(self, frame, bbox_xyxy, draw_trails, identities=None, categories=None,offset=(0,0)):
        ...
        for i, box in enumerate(bbox_xyxy):
            ...
            x2 += offset[0]
            y2 += offset[0]
            self.video_stat.increment()
            if self.webcam:
                self.video_stat.inc((i+1)*(.17 - (i*.01)))
            # print(f"Counter: {self.video_stat.count}")
    ...
    def run(self,
        ...
        if self.webcam:
            ...
            dataset = LoadStreams(self.video_stat,source, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
            bs = len(dataset)
        elif screenshot:
            dataset = LoadScreenshots(source, img_size=imgsz, stride=stride, auto=pt)
        else:
            print("rtsp")
            dataset = LoadImages(self.video_stat, source, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    if len(outputs) > 0:
                        ...
                        object_id = outputs[:, -1]
                        self.draw_boxes(ims, bbox_xyxy, self.video_stat.draw_trails, identities, object_id)
                        self.video_stat.set_frame(ims)
                    if self.video_stat.stop:
                        break
                    if self.video_stat.stop:
                        exit()
                # Stream results
